...
async function parseOpenAPISpec(content: string): Promise<{ 
  ...
  try {
    ...
    try {
      ...
      spec = dereferenced as OpenAPISpec;
...
function formatOpenAPISpec(spec: OpenAPISpec): string {
  const lines: string[] = [];
  ...
  if (spec.paths && Object.keys(spec.paths).length > 0) {
    ...
    Object.entries(spec.paths).forEach(([path, methods]) => {
      if (typeof methods === 'object' && methods !== null) {
        const methodList = Object.keys(methods).filter(key => 
          ['get', 'post', 'put', 'delete', 'patch', 'options', 'head', 'trace'].includes(key.toLowerCase())
        );
        ...
        if (methodList.length > 0) {
          ...
          methodList.forEach(method => {
            ...
            const summary = (operation?.summary as string) || (operation?.operationId as string) || `${method.toUpperCase()} operation`;
            lines.push(`- **${method.toUpperCase()}**: ${summary}`);
            
            if (operation?.description) {
              lines.push(`  - ${operation.description as string}`);
            }
          });
          lines.push('');
        }
...
function generateOpenAPISummary(spec: OpenAPISpec): string {
...
async function searchRelevantWebsites(query: string): Promise<SearchResult[]> {
  const config = getConfig();
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const websiteMatch = config.websites.find(site => 
      name === `fetch_${site.name.replace(/[^a-zA-Z0-9]/g, '_')}`
    );
